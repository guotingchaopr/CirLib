	...
	var init = function (configs) {
		...
		var animateImg = function (animate_configs) {
			for (var conf in animate_configs) {
				...
				img.src = "../global/img/" + (params.file == undefined ? params.name : params.file) + ".png";
		...
		var hangVal_polling = function (_data) {
			if (error_times[hangVal_URL] <= 0) {
				for (var i = 0; i < _data.length; i++) {
					...
					var obj = _data[i];
					if (obj.length == 1) {
						hang_elem.innerHTML = "<i style='color:" + obj[0].color + "'>" + obj[0].title + "</i><p>" + obj[0].value + "</p>"
					} else {
						for (var n = 0; n < obj.length; n++) {
							hang_elem.innerHTML += "<i style='color:" + obj[n].color + "'>" + obj[n].title + "</i><font>" + obj[n].value + "</font><br/>"
						}
					}
				}
			}
